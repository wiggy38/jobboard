<script lang="ts">
	...
	const countryName = $derived(COUNTRIES.find((c) => c.code === data.country)?.name ?? data.country);
	...
	let createForm = $state({
		...
		contractType: 'CDI', country: data.country, description: '', requirements: '',
	...
	async function submitCreate() {
		...
		try {
			const res = await adminApi.createOffer({
				...
				country: createForm.country || data.country,
	...
	function applyFilters() {
		goto(`/admin/offres/pays/${data.country.toLowerCase()}?${buildParams(1)}`);
	}

	function onTitleInput() {
		if (titleDebounce) clearTimeout(titleDebounce);
		titleDebounce = setTimeout(
			() => goto(`/admin/offres/pays/${data.country.toLowerCase()}?${buildParams(1)}`, { keepFocus: true, noScroll: true }),
			300
		);
	}
	...
	function resetFilters() {
		...
		title = '';
		city = '';
		goto(`/admin/offres/pays/${data.country.toLowerCase()}?page=1`);
	}

	function goPage(p: number) {
		goto(`/admin/offres/pays/${data.country.toLowerCase()}?${buildParams(p)}`);
	}

	const hasFilters = $derived(
		source !== '' || date !== '' || status !== '' || sector !== '' || score !== '' || title !== '' || city !== ''
	);
</script>

<div class="page">
	<div class="page-header">
		<h1>Offres d'emploi — {countryName}</h1>
		<button class="btn-create" onclick={openCreate}>+ Nouvelle offre</button>
	</div>

	<nav class="country-tabs">
		<a href="/admin/offres" class="country-tab">🌍 Toutes</a>
		{#each COUNTRIES as c}
			<a href="/admin/offres/pays/{c.code.toLowerCase()}" class="country-tab" class:active={c.code === data.country}>{c.name}</a>
		{/each}
	</nav>
	...
	<table>
		...
		<tbody>
			...
			{#if data.offers.length === 0}
				<tr><td colspan="8" class="empty">Aucune offre trouvée pour {countryName}.</td></tr>
			{/if}
		</tbody>
	</table>
	...
	<div class="modal-overlay" onclick={() => { if (!creating) showCreate = false; }}>
		<div class="modal" onclick={(e) => e.stopPropagation()}>
			<div class="modal-header">
				<h2>Nouvelle offre — {countryName}</h2>
