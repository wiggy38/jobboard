<script lang="ts">
	...
	let channelJoined = $state(data.filters.channelJoined ?? '');
	...
	function buildParams(page = 1) {
		...
		if (channelJoined) p.set('channelJoined', channelJoined);
		return p.toString();
	}

	function applyFilters() {
		goto(`/admin/abonnes?${buildParams(1)}`);
	}

	function onPhoneInput() {
		if (phoneDebounce) clearTimeout(phoneDebounce);
		phoneDebounce = setTimeout(
			() => goto(`/admin/abonnes?${buildParams(1)}`, { keepFocus: true, noScroll: true }),
			300
		);
	}
	...
	function resetFilters() {
		...
		phone = '';
		channelJoined = '';
		goto('/admin/abonnes?page=1');
	}

	function goPage(p: number) {
		goto(`/admin/abonnes?${buildParams(p)}`);
	}

	const hasFilters = $derived(
		plan !== '' || status !== '' || country !== '' || phone !== '' || channelJoined !== ''
	);
	...
	function hasJoinedChannel(user: AdminUser) {
		return user.channelJoins.some((c) => c.joined);
	}
</script>
...
<div class="page">
	...
	<div class="filters">
		<div class="filter-row">
			...
			<label class="filter-field">
				<span>Canal</span>
				<select bind:value={channelJoined}>
					<option value="">Tous</option>
					<option value="true">Rejoint</option>
					<option value="false">À relancer</option>
				</select>
			</label>
		</div>
	...
	<div class="results-summary">
		{data.total} abonné{data.total !== 1 ? 's' : ''} trouvé{data.total !== 1 ? 's' : ''}
	</div>
		...
		<tbody>
			{#each data.users as user}
				<tr onclick={() => goto(`/admin/abonnes/${user.id}`)} class="clickable">
					...
					<td>
						{#if hasJoinedChannel(user)}
							<span class="badge badge-channel-joined">✔ Rejoint</span>
						{:else}
							<span class="badge badge-channel-pending">À relancer</span>
						{/if}
					</td>
