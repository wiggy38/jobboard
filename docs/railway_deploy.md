...
| `tumaa-bot-scheduler` | `/` (racine repo) | `railway.bot-scheduler.json` | Worker BullMQ dédié (queue `bot`) pour `apps/bot/src/scheduler.ts` : jobs cron `channel-teaser-daily` (08h00) et `daily-digest` (07h30). **Service distinct de `tumaa-bot`** — `apps/bot/src/index.ts` (webhook, lancé par `tumaa-bot`) n'importe jamais `scheduler.ts` (voir CLAUDE.md, règle DAILY_DIGEST). Mêmes env vars que `tumaa-bot` (`DATABASE_URL`, `REDIS_URL`, `META_*`, `ANTHROPIC_API_KEY`, `PAYDUNYA_*`, `INTERNAL_API_URL`, `CHANNEL_INVITE_LINK_*`), sans `PORT` ni domaine public (pas de serveur HTTP) |
