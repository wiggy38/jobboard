...
6 services applicatifs partagent un seul repo. La plupart pointent vers leur propre
...
**Config as Code fermé aux nouveaux services depuis le 2026-08-28** (confirmé lors de la
création de `tumaa-bot-scheduler`, cf. ligne dédiée dans le tableau ci-dessous) : le champ
Config File Path rejette toute valeur pour un service qui n'a jamais utilisé Config as
Code, quelle que soit la syntaxe du chemin. Les services existants qui l'utilisaient déjà
(`tumaa-api`, `tumaa-bot`, `tumaa-scraper`, `tumaa-web-nginx`, `tumaa-web-app`,
`tumaa-home`) continuent de fonctionner ainsi jusqu'au 2026-12-01 (coupure totale
annoncée par Railway), mais **tout nouveau service doit être configuré directement dans
le dashboard** (Settings → Source → Root Directory, Settings → Build → Build Command,
Settings → Deploy → Start Command) sans fichier `railway.*.json` — voir `tumaa-bot-scheduler`
plus bas pour un exemple. Remplacement long terme annoncé par Railway : Infrastructure as
Code (`.railway/railway.ts`/`.railway/railway.py`, piloté par `railway config plan` en
CLI) — pas encore adopté ici, pas nécessaire tant que les services legacy tournent.
...
| `tumaa-scraper` | `/` (racine repo) | `railway.scraper.json` | Scheduler BullMQ (queue `scraper`) + Playwright (Chromium headless). Le build installe les libs système via `playwright install --with-deps` |
| `tumaa-bot-scheduler` | `/` (racine repo) | **Pas de Config as Code** (service créé après le 2026-08-28, champ rejeté — voir note en tête de page). Configuré directement en dashboard : Build Command = `pnpm install --frozen-lockfile && pnpm turbo run build --filter=...@tumaa/bot`, Start Command = `pnpm --filter @tumaa/bot scheduler`, Restart Policy = ON_FAILURE. `railway.bot-scheduler.json` retiré du repo (inutilisable, gardait la même intention documentée ici) | Worker BullMQ dédié (queue `bot`) pour `apps/bot/src/scheduler.ts` : jobs cron `channel-teaser-daily` (08h00) et `daily-digest` (07h30). **Service distinct de `tumaa-bot`** — `apps/bot/src/index.ts` (webhook, lancé par `tumaa-bot`) n'importe jamais `scheduler.ts` (voir CLAUDE.md, règle DAILY_DIGEST). Mêmes env vars que `tumaa-bot` (`DATABASE_URL`, `REDIS_URL`, `META_*`, `ANTHROPIC_API_KEY`, `PAYDUNYA_*`, `INTERNAL_API_URL`, `CHANNEL_INVITE_LINK_*`), sans `PORT` ni domaine public (pas de serveur HTTP) |
...
`tumaa-bot-scheduler` utilise le même builder RAILPACK et les mêmes commandes en
principe, mais réglées directement en dashboard plutôt que via JSON (voir note Config
as Code en tête de page).
